...
def embedding_sampling(unlabeled_loader, labelled_mean_embeddings, model, k = 10):
    ...
    values, indices = torch.topk(torch.tensor(embedding_cosine_similarity), k, largest=False)
    return torch.Tensor.tolist(indices)
...
def run_active_learning_loop(
    ...
    hidden_channels, num_layers, num_epochs,
     device,reinitialize_model = False
):
    test_acc, test_loss = [], []
...
def create_split_manager(dataset, seed, test_frac=0.3, init_frac=0.15, batch_size=8):
    set_seed(seed)
        ...
        for strat in args.strategies:
            for seed in args.seeds:
                for h in args.hidden_dims:
                    sm = create_split_manager(dataset, seed, batch_size=args.batch_size)
                    # set_seed(seed)
                    acc, loss = run_active_learning_loop(
